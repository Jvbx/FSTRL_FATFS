...
NRF_RESULT nrf_send_command(nrf24l01* dev, uint8_t cmd, const uint8_t* tx, uint8_t* rx, uint8_t len) {
...
void nrf_irq_handler(nrf24l01* dev) {
    ...
    if (nrf_read_register(dev, NRF_STATUS, (uint8_t*)&dev->registers.status) != NRF_OK) {dev->last_irq_result = NRF_ERROR; return; }

    if ((dev->registers.status & NRF_STATUS_RX_DR)) { // RX FIFO Interrupt
        nrf24_ce_reset(dev);
        uint8_t incoming_pipe = (dev->registers.status & NRF_STATUS_RX_P_NO) >> 1;
        ...
        if ((dev->registers.fifo_status & NRF_FIFO_STATUS_RX_EMPTY) == 0) {
            nrf_read_rx_payload(dev, (uint8_t*)&dev->config.pipes[incoming_pipe].rtxbuf, incoming_pipe);
            dev->registers.status |= NRF_STATUS_RX_DR;                                 // potentially datalost section. shoul be rewritten
            nrf_write_register(dev, NRF_STATUS, (uint8_t*)&dev->registers.status);
            // nrf_flush_rx(dev);
            nrf_packet_received_callback(dev, (uint8_t*)&dev->config.pipes[incoming_pipe].rtxbuf, incoming_pipe);
        }
        nrf24_ce_set(dev);
    }
    ...
    dev->last_irq_result = NRF_OK;
}

__weak void nrf_packet_received_callback(nrf24l01* dev, uint8_t* data, uint8_t pipenum) {
...
NRF_RESULT nrf_read_rx_payload(nrf24l01* dev, uint8_t* data, uint8_t pipenum) {
    ...
    uint8_t tx[dev->config.pipes[pipenum].pload_len];
    return nrf_send_command(dev, NRF_CMD_R_RX_PAYLOAD, tx, data, dev->config.pipes[pipenum].pload_len);
}
...
NRF_RESULT nrf_set_tx_address(nrf24l01* dev, const uint8_t* address) {
    ...
    if (nrf_send_command(dev, NRF_CMD_W_REGISTER | NRF_TX_ADDR, address, rx, dev->config.addr_width) != NRF_OK) {return NRF_ERROR;}
